# Estimated Marginal Means: Making Specific Comparisons
...
knitr::kable((exact_result$emm_results))
```
...
```{r}
knitr::kable(emmeans_power(exact_result$emmeans$contrasts))
```
...
knitr::kable(emmeans_power(pairs(simple_condition_effects)))
```
...
```{r}
knitr::kable(emmeans_power(test(simple_condition_effects, null = 0.5)))
```
...
knitr::kable(emmeans_power(voice_by_condition, alpha_level = 0.05 / 2))
```
...
```{r}
knitr::kable(monte_result1$emm_results)
```
