...
Let us assume we have a `4w` design with `mu = c(0,0.5,0.5,1)`, a common standard deviation of 1 (`sd=1`), and correlation between `a1` and `a4` of `r = .343` and a total sample size of 15 participants.
...
noncentrality = sqrt(((n*d^2) / (2*(1-min(rho)))))
...
exact_result$aov_result$Anova
exact_result$manova_results

Fdist2 <- FisherF(exact_result$aov_result$Anova$)
Fdist <- FisherF(3,12, noncentrality)
#Critical F
Ft <- quantile(Fdist, .95)
Ft
#Power
(1-pdf(Fdist, Ft))*100
