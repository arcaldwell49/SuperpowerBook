...
This is a compilation of documents for \href{https://github.com/arcaldwell49/Superpower}{Superpower} R package written in \textbf{Markdown} (Allaire et al. \protect\hyperlink{ref-R-rmarkdown}{2021}) and compiled by \textbf{Bookdown} (Xie \protect\hyperlink{ref-R-bookdown}{2020}).
...
This book was compiled with R version 4.0.2 (2020-06-22).
...
\NormalTok{power_sim <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(n, d, nsims, }\DataTypeTok{seed =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{print_plot =} \OtherTok{TRUE}\NormalTok{)\{}
  
\ControlFlowTok{if}\NormalTok{(seed }\OperatorTok{==}\StringTok{ }\OtherTok{TRUE}\NormalTok{)\{}\KeywordTok{set.seed}\NormalTok{(}\DecValTok{2}\NormalTok{)\}}
  
\CommentTok{# For sample size `n`, we want to perform `nsims` number of simulations of our experiment using effect size `d`}
  \KeywordTok{replicate}\NormalTok{(}\DataTypeTok{n =}\NormalTok{ nsims, }\DataTypeTok{exp =}\NormalTok{ \{}
\NormalTok{    x1 =}\StringTok{ }\KeywordTok{rnorm}\NormalTok{(n, d, }\DecValTok{1}\NormalTok{)}
\NormalTok{    x2 =}\StringTok{ }\KeywordTok{rnorm}\NormalTok{(n, }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{)}
    \KeywordTok{t.test}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ x1, }\DataTypeTok{y =}\NormalTok{ x2, }\DataTypeTok{var.equal =} \OtherTok{TRUE}\NormalTok{)}\OperatorTok{$}\NormalTok{p.value\}) ->}\StringTok{ }\NormalTok{ps}

\CommentTok{# Power for a given `n` is the long-run probability of rejecting the null hypothesis when it is true. In our simulation, it is the sum of all  `nsims` simulations of our study that yield p < .05 divided by the number of `nsims` performed. }
\NormalTok{  power =}\StringTok{ }\KeywordTok{sum}\NormalTok{(ps }\OperatorTok{<}\StringTok{ }\FloatTok{.05}\NormalTok{) }\OperatorTok{/}\StringTok{ }\NormalTok{nsims}
\NormalTok{  result =}\StringTok{ }\KeywordTok{cbind}\NormalTok{(n, power)}
  
\CommentTok{# Visualize the results as a plot}
\NormalTok{  ps }\OperatorTok{%>%}\StringTok{ }
\StringTok{    }\KeywordTok{as.data.frame}\NormalTok{() }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ .)) }\OperatorTok{+}
\StringTok{    }\KeywordTok{geom_histogram}\NormalTok{(}\DataTypeTok{binwidth =} \FloatTok{0.01}\NormalTok{) }\OperatorTok{+}
\StringTok{    }\KeywordTok{scale_x_continuous}\NormalTok{(}\DataTypeTok{breaks =}\NormalTok{ scales}\OperatorTok{::}\KeywordTok{pretty_breaks}\NormalTok{(}\DataTypeTok{n =} \DecValTok{15}\NormalTok{)) }\OperatorTok{+}
\StringTok{    }\KeywordTok{geom_vline}\NormalTok{(}\DataTypeTok{xintercept =} \FloatTok{0.05}\NormalTok{,}
               \DataTypeTok{linetype =} \StringTok{"dashed"}\NormalTok{,}
               \DataTypeTok{color =} \StringTok{"red"}\NormalTok{) }\OperatorTok{+}
\StringTok{    }\KeywordTok{xlab}\NormalTok{(}\DataTypeTok{label =} \KeywordTok{expression}\NormalTok{(}\KeywordTok{italic}\NormalTok{(p) }\OperatorTok{~}\StringTok{ "value"}\NormalTok{)) }\OperatorTok{+}
\StringTok{    }\KeywordTok{ylab}\NormalTok{(}\DataTypeTok{label =} \StringTok{"Frequency"}\NormalTok{) }\OperatorTok{+}
\StringTok{    }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{caption =} \KeywordTok{paste}\NormalTok{(}
      \StringTok{"Power ="}\NormalTok{,}
\NormalTok{      power }\OperatorTok{*}\StringTok{ }\DecValTok{100}\NormalTok{,}
      \StringTok{"% }\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{,}
      \KeywordTok{sum}\NormalTok{(ps }\OperatorTok{<}\StringTok{ }\FloatTok{.05}\NormalTok{),}
      \StringTok{"of"}\NormalTok{,}
\NormalTok{      nsims,}
      \StringTok{"simulations yield p < .05"}
\NormalTok{    )) }\OperatorTok{+}
\StringTok{    }\KeywordTok{ggtitle}\NormalTok{(}\DataTypeTok{label =} \KeywordTok{expr}\NormalTok{(}\KeywordTok{paste}\NormalTok{(}
      \StringTok{"N = "}\NormalTok{,}\OperatorTok{!!}\NormalTok{(n }\OperatorTok{*}\StringTok{ }\DecValTok{2}\NormalTok{),}
      \StringTok{" ("}\NormalTok{, n[}\DecValTok{1}\NormalTok{], }\StringTok{" = "}\NormalTok{,}\OperatorTok{!!}\NormalTok{n, }\StringTok{"; "}\NormalTok{, n[}\DecValTok{2}\NormalTok{], }\StringTok{" = "}\NormalTok{,}\OperatorTok{!!}\NormalTok{n, }\StringTok{")"}
\NormalTok{    ))) ->}\StringTok{ }\NormalTok{pl}
  
  \CommentTok{# Print our plot}
  \ControlFlowTok{if}\NormalTok{(print_plot }\OperatorTok{==}\StringTok{ }\OtherTok{TRUE}\NormalTok{)\{}\KeywordTok{print}\NormalTok{(pl)\}}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{power_sim}\NormalTok{(}\DataTypeTok{n =} \DecValTok{20}\NormalTok{, }\DataTypeTok{d =} \FloatTok{0.50}\NormalTok{, }\DataTypeTok{nsims =} \DecValTok{5000}\NormalTok{, }\DataTypeTok{seed =} \OtherTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-3-1}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{power_sim}\NormalTok{(}\DataTypeTok{n =} \DecValTok{50}\NormalTok{, }\DataTypeTok{d =} \FloatTok{0.50}\NormalTok{, }\DataTypeTok{nsims =} \DecValTok{5000}\NormalTok{, }\DataTypeTok{seed =} \OtherTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-4-1}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{power_sim}\NormalTok{(}\DataTypeTok{n =} \DecValTok{80}\NormalTok{, }\DataTypeTok{d =} \FloatTok{0.50}\NormalTok{, }\DataTypeTok{nsims =} \DecValTok{5000}\NormalTok{, }\DataTypeTok{seed =} \OtherTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-5-1}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pwr}\OperatorTok{::}\KeywordTok{pwr.t.test}\NormalTok{(}\DataTypeTok{n =} \DecValTok{80}\NormalTok{, }\DataTypeTok{d =} \FloatTok{0.50}\NormalTok{, }\DataTypeTok{sig.level =} \FloatTok{0.05}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Where \(M_1\) is the mean of group one, \(M_2\) is the mean of group two, and \(SD_{pool}\) is the pooled standard deviation of group one and two. Cohen's \emph{d} is sometimes given descriptive labels of small, moderate, and large that correspond to 0.2, 0.5, and 0.8, respectively. However, these descriptives can differ across different measurements. We would strongly advocate against using these ``default'' interpretation scales and instead understand your phenomena/measurements well enough to understand when an effect is meaningful (Caldwell and Vigotsky \protect\hyperlink{ref-Caldwell2020}{2020}). Some additional common effect sizes alongside their descriptive magnitudes are given in the following table:
...
\NormalTok{group_}\DecValTok{1}\NormalTok{ =}\StringTok{ }\KeywordTok{rnorm}\NormalTok{(}\DataTypeTok{n =} \DecValTok{20}\NormalTok{, }\DataTypeTok{mean =} \FloatTok{0.5}\NormalTok{, }\DataTypeTok{sd =} \DecValTok{1}\NormalTok{)}
\NormalTok{group_}\DecValTok{2}\NormalTok{ =}\StringTok{ }\KeywordTok{rnorm}\NormalTok{(}\DataTypeTok{n =} \DecValTok{20}\NormalTok{, }\DataTypeTok{mean =} \DecValTok{0}\NormalTok{, }\DataTypeTok{sd =} \DecValTok{1}\NormalTok{)}

\CommentTok{# Visualize the data}
\KeywordTok{data.frame}\NormalTok{(group_}\DecValTok{1}\NormalTok{, group_}\DecValTok{2}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{tidyr}\OperatorTok{::}\KeywordTok{pivot_longer}\NormalTok{(}
    \DataTypeTok{names_to =} \StringTok{"group"}\NormalTok{, }
    \DataTypeTok{values_to =} \StringTok{"dv"}\NormalTok{, }
    \DataTypeTok{cols =} \KeywordTok{everything}\NormalTok{()) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{GGally}\OperatorTok{::}\KeywordTok{ggpairs}\NormalTok{(ggplot2}\OperatorTok{::}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{colour =}\NormalTok{ group), }
                  \DataTypeTok{lower =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{combo =} \KeywordTok{wrap}\NormalTok{(ggally_facethist, }
                                            \DataTypeTok{binwidth =} \FloatTok{0.5}\NormalTok{)))}
\end{Highlighting}
\end{Shaded}

\includegraphics[width=1\linewidth]{SuperpowerValidation_files/figure-latex/unnamed-chunk-8-1}
...
\NormalTok{n1 =}\StringTok{ }\KeywordTok{length}\NormalTok{(group_}\DecValTok{1}\NormalTok{)}
\NormalTok{n2 =}\StringTok{ }\KeywordTok{length}\NormalTok{(group_}\DecValTok{2}\NormalTok{)}

\CommentTok{# Determine standard deviation for each group}
\NormalTok{sd1 =}\StringTok{ }\KeywordTok{sd}\NormalTok{(group_}\DecValTok{1}\NormalTok{)}
\NormalTok{sd2 =}\StringTok{ }\KeywordTok{sd}\NormalTok{(group_}\DecValTok{2}\NormalTok{)}

\CommentTok{# Compute pooled standard deviation}
\NormalTok{sd_pool =}\StringTok{ }\KeywordTok{sqrt}\NormalTok{((sd1 }\OperatorTok{*}\StringTok{ }\NormalTok{sd1 }\OperatorTok{*}\StringTok{ }\NormalTok{(n1 }\OperatorTok{-}\StringTok{ }\DecValTok{1}\NormalTok{) }\OperatorTok{+}\StringTok{ }\NormalTok{sd2 }\OperatorTok{*}\StringTok{ }\NormalTok{sd2 }\OperatorTok{*}\StringTok{ }\NormalTok{(n2 }\OperatorTok{-}\StringTok{ }\DecValTok{1}\NormalTok{)) }\OperatorTok{/}\StringTok{ }\NormalTok{(n1 }\OperatorTok{+}\StringTok{ }\NormalTok{n2 }\OperatorTok{-}\StringTok{ }\DecValTok{2}\NormalTok{))}

\CommentTok{# Determine the difference between the means}
\NormalTok{m_diff =}\StringTok{ }\KeywordTok{mean}\NormalTok{(group_}\DecValTok{1}\NormalTok{) }\OperatorTok{-}\StringTok{ }\KeywordTok{mean}\NormalTok{(group_}\DecValTok{2}\NormalTok{)}

\CommentTok{# Compute Cohen's d}
\NormalTok{d =}\StringTok{ }\NormalTok{m_diff }\OperatorTok{/}\StringTok{ }\NormalTok{sd_pool}

\CommentTok{# Compute 95% CI on Cohen's d}
\NormalTok{(}\DataTypeTok{d_ci =}\NormalTok{ MBESS}\OperatorTok{::}\KeywordTok{ci.smd}\NormalTok{(}\DataTypeTok{smd =}\NormalTok{ d, }\DataTypeTok{n.1 =}\NormalTok{ n1, }\DataTypeTok{n.2 =}\NormalTok{ n2, }\DataTypeTok{conf.level =} \FloatTok{0.95}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{d_ci }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{as.data.frame}\NormalTok{() }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =} \StringTok{"Cohen's d"}\NormalTok{)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_point}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{y =}\NormalTok{ smd)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_errorbar}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{ymin =}\NormalTok{ Lower.Conf.Limit.smd,}
                    \DataTypeTok{ymax =}\NormalTok{ Upper.Conf.Limit.smd), }\DataTypeTok{width =} \FloatTok{0.1}\NormalTok{) }\OperatorTok{+}
...
\StringTok{  }\KeywordTok{ylab}\NormalTok{(}\DataTypeTok{label =} \StringTok{"Cohen's d"}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{caption =} \StringTok{"Note: Error bar = 95% CI"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-10-1}
...
Perugini, Gallucci, and Costantini (\protect\hyperlink{ref-Perugini2014}{2014}) suggest we take into account the uncertainty of the sample effect size. To do so, they suggest for the researcher to compute the lower absolute limit of a 60\% CI on the sample effect. This value would then be used as a \textbf{\emph{safeguard effect size}} and inputted into a power analysis. Why 60\%? Well, the lower limit of a 60\% CI corresponds to a one-sided 80\% CI. As Perugini, Gallucci, and Costantini (\protect\hyperlink{ref-Perugini2014}{2014}) describe, this implies a 20\% risk that that the true population effect size is lower than this limit, and correspondingly, 80\% assurance that the population effect size is equal to or greater than this limit. We focus on the lower boundary because of the asymmetry of overestimating compared to underestimating the population effect size in a power analysis.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{(}\DataTypeTok{d_safeguard =}\NormalTok{ MBESS}\OperatorTok{::}\KeywordTok{ci.smd}\NormalTok{(}\DataTypeTok{smd =}\NormalTok{ d, }\DataTypeTok{n.1 =}\NormalTok{ n1, }\DataTypeTok{n.2 =}\NormalTok{ n2, }\DataTypeTok{conf.level =} \FloatTok{0.60}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
All authors of this book are in agreement: \emph{post hoc} power analysis is useless. Many statisticians have made this abundantly clear (Gelman \protect\hyperlink{ref-gelman2019don}{2019}; Althouse \protect\hyperlink{ref-Althouse2021}{2021}).
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{t.test}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ group_}\DecValTok{1}\NormalTok{, }\DataTypeTok{y =}\NormalTok{ group_}\DecValTok{2}\NormalTok{, }\DataTypeTok{var.equal =} \OtherTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Remind ourselves of the value of Cohen's d for these data:}
\NormalTok{(d)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Perform the post hoc power analysis by excluding `power`. Note: `n` refers to sample size 'per group':}
\NormalTok{pwr}\OperatorTok{::}\KeywordTok{pwr.t.test}\NormalTok{(}\DataTypeTok{n =} \DecValTok{20}\NormalTok{, }\DataTypeTok{d =}\NormalTok{ d, }\DataTypeTok{sig.level =} \FloatTok{0.05}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{delta =}\StringTok{ }\FloatTok{0.5}

\CommentTok{# Perform the post hoc power analysis by excluding `power`:}
\NormalTok{pwr}\OperatorTok{::}\KeywordTok{pwr.t.test}\NormalTok{(}\DataTypeTok{n =} \DecValTok{20}\NormalTok{, }\DataTypeTok{d =}\NormalTok{ delta, }\DataTypeTok{sig.level =} \FloatTok{0.05}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\emph{Post hoc} power analyses are simply a re-expression of the obtained \emph{p} value. As Lenth (\protect\hyperlink{ref-Lenth2007}{2007}) describes, to compute \emph{post hoc} power all we need (beyond our chosen \(\alpha\)) is the \emph{p} value of the test and associated degrees of freedom. For this reason, \emph{post hoc} power does not provide more information than that which is already provided by our \emph{p} value.

Recall from above that our obtained \emph{p} value in our simulated data is .057. Lenth (\protect\hyperlink{ref-Lenth2007}{2007}) provides a simple R function to compute \emph{post hoc} power from the obtained \emph{p} value from an independent groups \emph{t} test as follows:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Define power function}
\NormalTok{power <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(p_val, deg_f, alpha)\{}

\NormalTok{  delta =}\StringTok{ }\KeywordTok{qt}\NormalTok{(}\DataTypeTok{p =} \DecValTok{1} \OperatorTok{-}\StringTok{ }\NormalTok{p_val }\OperatorTok{/}\StringTok{ }\DecValTok{2}\NormalTok{, }\DataTypeTok{df =}\NormalTok{ deg_f)}
\NormalTok{  crit_val =}\StringTok{ }\KeywordTok{qt}\NormalTok{(}\DataTypeTok{p =} \DecValTok{1} \OperatorTok{-}\StringTok{ }\NormalTok{alpha }\OperatorTok{/}\StringTok{ }\DecValTok{2}\NormalTok{, }\DataTypeTok{df =}\NormalTok{ deg_f)}
\NormalTok{  power =}\StringTok{ }\DecValTok{1} \OperatorTok{-}\StringTok{ }\KeywordTok{pt}\NormalTok{(}\DataTypeTok{q =}\NormalTok{ crit_val, }\DataTypeTok{df =}\NormalTok{ deg_f, }\DataTypeTok{ncp =}\NormalTok{ delta) }\OperatorTok{+}\StringTok{ }\KeywordTok{pt}\NormalTok{(}\OperatorTok{-}\NormalTok{crit_val, deg_f, delta)}
...
\KeywordTok{power}\NormalTok{(}\DataTypeTok{p_val =} \FloatTok{0.05707}\NormalTok{, }\DataTypeTok{deg_f =} \DecValTok{38}\NormalTok{, }\DataTypeTok{alpha =} \FloatTok{.05}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{v =}\StringTok{ }\KeywordTok{list}\NormalTok{(}\KeywordTok{seq}\NormalTok{(}\DecValTok{20}\NormalTok{,}\DecValTok{200}\NormalTok{,}\DecValTok{20}\NormalTok{), }\OtherTok{Inf}\NormalTok{)}
\KeywordTok{map_df}\NormalTok{(v, }\OperatorTok{~}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(., }
                       \KeywordTok{power}\NormalTok{(}\DataTypeTok{p_val =} \FloatTok{.05}\NormalTok{, }\DataTypeTok{deg_f =}\NormalTok{ ., }\DataTypeTok{alpha =} \FloatTok{.05}\NormalTok{), }
                       \KeywordTok{power}\NormalTok{(}\DataTypeTok{p_val =} \FloatTok{0.10}\NormalTok{, }\DataTypeTok{deg_f =}\NormalTok{ ., }\DataTypeTok{alpha =} \FloatTok{.05}\NormalTok{),}
                       \KeywordTok{power}\NormalTok{(}\DataTypeTok{p_val =} \FloatTok{0.20}\NormalTok{, }\DataTypeTok{deg_f =}\NormalTok{ ., }\DataTypeTok{alpha =} \FloatTok{.05}\NormalTok{))) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  `}\DataTypeTok{colnames<-}\StringTok{`}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\StringTok{"Degrees of Freedom"}\NormalTok{, }
                 \StringTok{"Power ($p$ = .05)"}\NormalTok{,}
                 \StringTok{"Power ($p$ = .10)"}\NormalTok{, }
                 \StringTok{"Power ($p$ = .20)"}\NormalTok{)) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\DataTypeTok{escape =} \OtherTok{FALSE}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{kableExtra}\OperatorTok{::}\KeywordTok{kable_styling}\NormalTok{(}\DataTypeTok{bootstrap_options =} \StringTok{"striped"}\NormalTok{, }
                            \DataTypeTok{full_width =}\NormalTok{ T, }
...
\NormalTok{deg_f =}\StringTok{ }\NormalTok{n1 }\OperatorTok{+}\StringTok{ }\NormalTok{n2 }\OperatorTok{-}\StringTok{ }\DecValTok{2}
\NormalTok{(}\DataTypeTok{t_crit =} \KeywordTok{qt}\NormalTok{(}\DataTypeTok{p =} \DecValTok{1} \OperatorTok{-}\StringTok{ }\NormalTok{(alpha}\OperatorTok{/}\DecValTok{2}\NormalTok{), }\DataTypeTok{df =}\NormalTok{ deg_f))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{result =}\StringTok{ }\KeywordTok{t.test}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ group_}\DecValTok{1}\NormalTok{, }\DataTypeTok{y =}\NormalTok{ group_}\DecValTok{2}\NormalTok{, }\DataTypeTok{var.equal =} \OtherTok{TRUE}\NormalTok{)}

\CommentTok{# Extract the t value}
\NormalTok{(}\DataTypeTok{t_val =} \KeywordTok{as.numeric}\NormalTok{(result}\OperatorTok{$}\NormalTok{statistic))}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{# Compute Cohen's d from the t statistic and sample sizes alone}
\NormalTok{t_val }\OperatorTok{*}\StringTok{ }\KeywordTok{sqrt}\NormalTok{((}\DecValTok{1}\OperatorTok{/}\NormalTok{n1 }\OperatorTok{+}\StringTok{ }\DecValTok{1}\OperatorTok{/}\NormalTok{n2))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Remind ourselves of what t_crit is}
\NormalTok{(t_crit)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{t_crit }\OperatorTok{*}\StringTok{ }\KeywordTok{sqrt}\NormalTok{((}\DecValTok{1}\OperatorTok{/}\NormalTok{n1 }\OperatorTok{+}\StringTok{ }\DecValTok{1}\OperatorTok{/}\NormalTok{n2))}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-21-1}
...
a2\_b2 & 0.93 & 0.95 & 0.96 & 1.00\tabularnewline
\bottomrule
\end{longtable}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-23-1}
\newpage
...
\caption{\label{tab:unnamed-chunk-27}Covariance-Variance Matrix}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & red\_fast & red\_slow & blue\_fast & blue\_slow\\
...
Using the formula also used by Albers and Lakens (\protect\hyperlink{ref-albers2018power}{2018}), we can determine the means that should yield a specified effect sizes (expressed in Cohen's \emph{f}). Eta-squared (identical to partial eta-squared for one-way ANOVA's) has benchmarks of .0099, .0588, and .1379 for small, medium, and large effect sizes (Cohen \protect\hyperlink{ref-cohen1988spa}{1988}). Although these benchmarks are quite arbitrary, and researchers should only use such benchmarks for power analyses as a last resort, we will demonstrate an a priori power analysis for these values.

\hypertarget{two-conditions}{%
\subsection{Two conditions}\label{two-conditions}}

Imagine we aim to design a study to test the hypothesis that giving people a pet to take care of will increase their life satisfaction. We have a control condition, and a condition where people get a pet, and randomly assign participants to either condition. We can simulate a one-way ANOVA with a specified alpha, sample size, and effect size, on see the statistical power we would have for the ANOVA and the follow-up comparisons. We expect pets to increase life-satisfaction compared to the control condition. Based on work by Pavot and Diener (\protect\hyperlink{ref-pavot1993affective}{1993}) we believe that we can expect responses on the life-satifaction scale to have a mean of approximately 24 in our population, with a standard deviation of 6.4. We expect having a pet increases life satisfaction with approximately 2.2 scale points for participants who get a pet. There are 200 participants in total, with 100 participants in each condition. But before we proceed with the data collection, we examine the statistical power our design would have to detect the differences we predict.
...
\includegraphics{SuperpowerValidation_files/figure-latex/start_one_way-1}
...
\caption{\label{tab:unnamed-chunk-30}Simulated ANOVA Result}
\centering
...
\caption{\label{tab:unnamed-chunk-32}Exact ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
...
We can also directly compute Cohen's \emph{f} from Cohen's d for two groups, as Cohen (\protect\hyperlink{ref-cohen1988spa}{1988}) describes, because \emph{f} = 1/2d. So \emph{f} = 0.5*0.34375 = 0.171875. And indeed, power analysis using the pwr package yields the same result using the \texttt{pwr.anova.test} as the \texttt{power.t.test}.
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-38-1}
...
\caption{\label{tab:unnamed-chunk-40}Simulated ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r}
\hline
  & power & effect\_size\\
...
\caption{\label{tab:unnamed-chunk-42}Exact ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-44-1}
...
\caption{\label{tab:unnamed-chunk-46}Simulated ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r}
\hline
  & power & effect\_size\\
...
\caption{\label{tab:unnamed-chunk-48}Exact ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-49-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-50-1}
\newpage
\#\# Part 2
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-51-1}
...
This sensitivity analysis shows we have 90\% power in our planned design to detect effects of Cohen's \emph{f} of 0.2934417. Benchmarks by Cohen (\protect\hyperlink{ref-cohen1988spa}{1988}) for small, medium, and large Cohen's \emph{f} values are 0.1, 0.25, and 0.4, which correspond to eta-squared values of small (.0099), medium (.0588), and large (.1379), in line with d = .2, .5, or .8. So, at least based on these benchmarks, we have 90\% power to detect effects that are somewhat sizeable.
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-60-1}
...
Using the formulas below, we can calculate the means for a one-way ANOVA. Using the formula from Albers and Lakens (\protect\hyperlink{ref-albers2018power}{2018}), we can determine the means that should yield a specified effect sizes (expressed in Cohen's \emph{f}).

Eta-squared (idential to partial eta-squared for one-way ANOVA's) has benchmarks of .0099, .0588, and .1379 for small, medium, and large effect sizes (Cohen \protect\hyperlink{ref-cohen1988spa}{1988}).
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-67-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-73-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-79-1}
...
There is no equivalent ``fz'' for Cohen's \emph{f} for a within subject ANOVA. For two groups, we can directly compute Cohen's \emph{f} from Cohen's d for two groups, as Cohen (\protect\hyperlink{ref-cohen1988spa}{1988}) describes, because f = 1/2d. For a d = 0.5, f = 0.25. In g*power we can run a 2 group within-subject power analysis for ANOVA. We plan for 80\% power, and reproduce the anaysis above for the dependent \emph{t}-test. This works because the correlation is set to 0.5, when d = dz, and thus the transformation of f=1/2d works.
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-85-1}
...
\caption{\label{tab:unnamed-chunk-87}Simulated ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r}
\hline
  & power & effect\_size\\
...
\caption{\label{tab:unnamed-chunk-89}Exact ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-90-1}
...
\caption{\label{tab:unnamed-chunk-91}Covariance-Variance Matrix}
\centering
\begin{tabular}[t]{l|r|r}
\hline
  & fast & slow\\
...
\caption{\label{tab:unnamed-chunk-93}Simulated ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r}
\hline
  & power & effect\_size\\
...
\caption{\label{tab:unnamed-chunk-95}Exact ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
following Cohen (\protect\hyperlink{ref-cohen1988spa}{1988}), p 277. The patterns are:
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-99-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-104-1}
...
As Potvin and Schutz (\protect\hyperlink{ref-potvin2000statistical}{2000}) explain, analytic procedures for a two-factor repeated measures ANOVA do not seem to exist. The main problem is quantifying the error variance (the denominator when calculating lambda or Cohen's \emph{f}). Simulation based aproaches provide a solution.
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-114-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-115-1}
...
\caption{\label{tab:unnamed-chunk-117}Simulated ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r}
\hline
  & power & effect\_size\\
...
\caption{\label{tab:unnamed-chunk-119}Exact ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-124-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-125-1}
...
\caption{\label{tab:unnamed-chunk-127}Simulated ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r}
\hline
  & power & effect\_size\\
...
\caption{\label{tab:unnamed-chunk-129}Exact ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
Potvin and Schutz (\protect\hyperlink{ref-potvin2000statistical}{2000}) simulate a wide range of repeated measure designs. They give an example of a 3x3 design, with the following correlation matrix:
...
Potvin and Schutz (\protect\hyperlink{ref-potvin2000statistical}{2000}) examine power for 2x2 within ANOVA designs and develop approximations of the error variance. For a design with 2 within factors (A and B) these are:
...
Let's now compare the formulas in Potvin and Schutz (\protect\hyperlink{ref-potvin2000statistical}{2000}) with \texttt{Superpower} with a simple scenario.
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-130-1}
...
We can try to use the formula in Potvin and Schutz (\protect\hyperlink{ref-potvin2000statistical}{2000}).
...
\caption{\label{tab:unnamed-chunk-136}Analytical Result}
\centering
...
Let's now extend to one of the simlations presented by Potvin and Schutz (\protect\hyperlink{ref-potvin2000statistical}{2000}) with a 3x3 solution.
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-137-1}
...
The design now matches the correlation matrix in Figure 1 of Potvin and Schutz (\protect\hyperlink{ref-potvin2000statistical}{2000}).
...
\caption{\label{tab:unnamed-chunk-139}Exact ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
\caption{\label{tab:unnamed-chunk-143}Analytical Result}
\centering
\begin{tabular}[t]{r|r|r}
\hline
variance & lambda & power\\
...
A large proportion of research with within-subjects manipulations, or repeated measures, rely upon the ``univariate'' approach (Maxwell, Delaney, and Kelley \protect\hyperlink{ref-maxwell_designing_2004}{2004}). While this approach is valid, when corrections for sphercity are applied, it may not be the most powerful or informative analysis plan. Instead, researchers should consider a multivariate analysis (MANOVA). While the MANOVA is not ``assumption free'' it does not assume sphercity which makes it a very attractive analytical tool and the preferred method of analysis for some situations (Maxwell, Delaney, and Kelley \protect\hyperlink{ref-maxwell_designing_2004}{2004}).

For a simple one-way repeated measures design, there are some simple guidelines for power analysis set forth by Maxwell, Delaney, and Kelley (\protect\hyperlink{ref-maxwell_designing_2004}{2004}) (pg. 750). All that is needed in the effect size calculated as:
...
In addition, the non-centrality parameter of the \emph{F}-statistic can be estimated from Vonesh and Schork (\protect\hyperlink{ref-vonesh1986sample}{1986}) equations as the following:
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-146-1}
...
\caption{\label{tab:unnamed-chunk-147}MANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
...
We can simulate a two-way ANOVA with a specific alpha, sample size and effect size, to achieve a specified statistical power. We will try to reproduce the power analysis in g*power (Faul et al. \protect\hyperlink{ref-faul2007g}{2007}) for an F-test from an ANOVA with a repeated measures, within-between interaction effect. While g*power is a great tool it has limited options for mixed factorial ANOVAs.
\newpage 
...
\includegraphics{SuperpowerValidation_files/figure-latex/start_mixedANOVA-1}
...
\caption{\label{tab:unnamed-chunk-149}Simulated ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r}
\hline
  & power & effect\_size\\
...
\caption{\label{tab:unnamed-chunk-151}Exact ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-152-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-159-1}
...
\caption{\label{tab:unnamed-chunk-161}MANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
Now we move onto the example from pg 3794 of SAS (\protect\hyperlink{ref-SASglmpower}{2015}).
...
Then we can run the analysis in SAS. Note that in the example SAS (\protect\hyperlink{ref-SASglmpower}{2015}) are assuming a linear exponential covariance matrix, which we can mimic in \texttt{R}.
...
\includegraphics{SuperpowerValidation_files/figure-latex/mixed_multivariate_1-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-165-1}
...
\caption{\label{tab:unnamed-chunk-166}Simulated MANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & pillai\_trace & cohen\_f & non\_centrality\\
...
\includegraphics{SuperpowerValidation_files/figure-latex/start_threewayinteraction-1}
...
\caption{\label{tab:unnamed-chunk-169}Simulated ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r}
\hline
  & power & effect\_size\\
...
\caption{\label{tab:unnamed-chunk-171}Exact ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-174-1}
...
\caption{\label{tab:unnamed-chunk-176}Simulated ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r}
\hline
  & power & effect\_size\\
...
\caption{\label{tab:unnamed-chunk-178}Exact ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-180-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-186-1}
...
\caption{\label{tab:unnamed-chunk-190}Exact ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-192-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-195-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-198-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-202-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-218-1}
...
\caption{\label{tab:unnamed-chunk-218}ANOVA Results}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-218-2}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-225-1}
...
\caption{\label{tab:unnamed-chunk-225}ANOVA Results}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-226-1}
...
\caption{\label{tab:unnamed-chunk-226}ANOVA Results}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
\NormalTok{string <-}\StringTok{ "2b*2b"}
\NormalTok{n <-}\StringTok{ }\DecValTok{300}
\NormalTok{mu <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{25}\NormalTok{, }\DecValTok{20}\NormalTok{, }\DecValTok{20}\NormalTok{, }\DecValTok{25}\NormalTok{) }\CommentTok{#All marginal means are equal - so there is no real difference.}
\CommentTok{# Enter means in the order that matches the labels below.}
\NormalTok{sd <-}\StringTok{ }\DecValTok{20}
\NormalTok{labelnames <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"fruit"}\NormalTok{, }\StringTok{"apple"}\NormalTok{, }\StringTok{"banana"}\NormalTok{, }
                \StringTok{"hunger"}\NormalTok{, }\StringTok{"no hunger"}\NormalTok{, }\StringTok{"very hungry"}\NormalTok{) }\CommentTok{#}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-232-1}
...
\caption{\label{tab:unnamed-chunk-232}ANOVA Results}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
\includegraphics{SuperpowerValidation_files/figure-latex/variations_in_design-1}
...
\caption{\label{tab:unnamed-chunk-236}Simulated ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r}
\hline
  & power & effect\_size\\
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-239-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-240-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-242-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-243-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-244-1}
...
\NormalTok{mean_mat <-}\StringTok{ }\KeywordTok{t}\NormalTok{(}\KeywordTok{matrix}\NormalTok{(mu, }\DataTypeTok{nrow =} \DecValTok{2}\NormalTok{, }\DataTypeTok{ncol =} \DecValTok{1}\NormalTok{)) }\CommentTok{#Create a mean matrix}
...
\NormalTok{sd =}\StringTok{ }\DecValTok{10}
\NormalTok{n <-}\StringTok{ }\DecValTok{50}
\NormalTok{mean_mat <-}\StringTok{ }\KeywordTok{t}\NormalTok{(}\KeywordTok{matrix}\NormalTok{(mu, }\DataTypeTok{nrow =} \DecValTok{3}\NormalTok{, }\DataTypeTok{ncol =} \DecValTok{1}\NormalTok{)) }\CommentTok{#Create a mean matrix}
\CommentTok{# Using the sweep function to remove rowmeans from the matrix}
\NormalTok{mean_mat_res <-}\StringTok{ }\KeywordTok{sweep}\NormalTok{(mean_mat, }\DecValTok{2}\NormalTok{, }\KeywordTok{rowMeans}\NormalTok{(mean_mat))}
\NormalTok{mean_mat_res}
\end{Highlighting}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-247-1}
...
\caption{\label{tab:unnamed-chunk-249}Exact ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-250-1}
...
\NormalTok{r <-}\StringTok{ }\FloatTok{0.5}
\NormalTok{string <-}\StringTok{ "2w"}
\NormalTok{mu <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{5}\NormalTok{) }
\NormalTok{design_result <-}\StringTok{ }\KeywordTok{ANOVA_design}\NormalTok{(}
  \DataTypeTok{design =}\NormalTok{ string,}
  \DataTypeTok{n =}\NormalTok{ n,}
  \DataTypeTok{mu =}\NormalTok{ mu,}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-253-1}
...
\begin{Highlighting}[]
\NormalTok{string <-}\StringTok{ "3w"}
\NormalTok{mu <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{5}\NormalTok{)}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-253-2}
...
\NormalTok{mu <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{5}\NormalTok{)}
\NormalTok{design_result <-}\StringTok{ }\KeywordTok{ANOVA_design}\NormalTok{(}
  \DataTypeTok{design =}\NormalTok{ string,}
  \DataTypeTok{n =}\NormalTok{ n,}
  \DataTypeTok{mu =}\NormalTok{ mu,}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-253-3}
...
\NormalTok{mu <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{5}\NormalTok{) }
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-253-4}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-253-5}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-253-6}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(}\KeywordTok{paste0}\NormalTok{(}\StringTok{"power: "}\NormalTok{, }\KeywordTok{power_oneway_within}\NormalTok{(design_result)}\OperatorTok{$}\NormalTok{power, }\StringTok{"}\CharTok{\textbackslash{}n}\StringTok{Cohen's F: "}\NormalTok{, }\KeywordTok{power_oneway_within}\NormalTok{(design_result)}\OperatorTok{$}\NormalTok{Cohen_f,}
             \StringTok{"}\CharTok{\textbackslash{}n}\StringTok{Cohen's F (SPSS): "}\NormalTok{, }\KeywordTok{power_oneway_within}\NormalTok{(design_result)}\OperatorTok{$}\NormalTok{Cohen_f_SPSS,}
             \StringTok{"}\CharTok{\textbackslash{}n}\StringTok{lambda: "}\NormalTok{, }\KeywordTok{power_oneway_within}\NormalTok{(design_result)}\OperatorTok{$}\NormalTok{lambda,}
             \StringTok{"}\CharTok{\textbackslash{}n}\StringTok{Critical F: "}\NormalTok{,}
             \KeywordTok{power_oneway_within}\NormalTok{(design_result)}\OperatorTok{$}\NormalTok{F_critical}
\NormalTok{             ))}
\end{Highlighting}
\end{Shaded}
...
\begin{verbatim}
## [1] "power: 75.4600950274594\nCohen's F: 0.174963553055941\nCohen's F (SPSS): 0.271830141109781\nlambda: 12.8571428571429\nCritical F: 2.15101618343483"
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# power_oneway_within(design_result)$power}
\CommentTok{# power_oneway_within(design_result)$Cohen_f}
\CommentTok{# power_oneway_within(design_result)$Cohen_f_SPSS}
\CommentTok{# power_oneway_within(design_result)$lambda}
\CommentTok{# power_oneway_within(design_result)$F_critical}
\end{Highlighting}
\end{Shaded}
...
When using power analysis software such as g*power (Faul et al. \protect\hyperlink{ref-faul2007g}{2007}), this is important to realize.
...
\includegraphics{SuperpowerValidation_files/figure-latex/error_control-1}
...
\caption{\label{tab:unnamed-chunk-256}Simulated ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r}
\hline
  & power & effect\_size\\
...
\KeywordTok{apply}\NormalTok{(}\KeywordTok{as.matrix}\NormalTok{(simulation_result_}\FloatTok{8.1}\OperatorTok{$}\NormalTok{sim_data[(}\DecValTok{1}\OperatorTok{:}\DecValTok{7}\NormalTok{)]), }\DecValTok{2}\NormalTok{, }
    \ControlFlowTok{function}\NormalTok{(x) }\KeywordTok{round}\NormalTok{(}\KeywordTok{mean}\NormalTok{(}\KeywordTok{ifelse}\NormalTok{(x }\OperatorTok{<}\StringTok{ }\NormalTok{alpha_level, }\DecValTok{1}\NormalTok{, }\DecValTok{0}\NormalTok{)),}\DecValTok{4}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{sum}\NormalTok{(}\KeywordTok{apply}\NormalTok{(}\KeywordTok{as.matrix}\NormalTok{(simulation_result_}\FloatTok{8.1}\OperatorTok{$}\NormalTok{sim_data[(}\DecValTok{1}\OperatorTok{:}\DecValTok{7}\NormalTok{)]), }\DecValTok{1}\NormalTok{, }
    \ControlFlowTok{function}\NormalTok{(x) }
      \KeywordTok{round}\NormalTok{(}\KeywordTok{mean}\NormalTok{(}\KeywordTok{ifelse}\NormalTok{(x }\OperatorTok{<}\StringTok{ }\NormalTok{alpha_level, }\DecValTok{1}\NormalTok{, }\DecValTok{0}\NormalTok{)),}\DecValTok{4}\NormalTok{)) }\OperatorTok{>}\StringTok{ }\DecValTok{0}\NormalTok{) }\OperatorTok{/}\StringTok{ }\NormalTok{nsims}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-259-1}
...
\caption{\label{tab:unnamed-chunk-261}ANOVA Results}
\centering
\begin{tabular}[t]{l|r|r}
\hline
  & power & effect\_size\\
...
\caption{\label{tab:unnamed-chunk-262}Pairwise Results}
\centering
\begin{tabular}[t]{l|r|r}
\hline
  & power & effect\_size\\
...
Alternative procedures to control error rates can be found in the \href{https://cran.r-project.org/web/packages/multcomp/index.html}{multcomp} R package (Hothorn, Bretz, and Westfall \protect\hyperlink{ref-R-multcomp}{2020}).
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-264-1}
...
This is a generalized function for one-way ANOVA's for any number of groups. It is in part based on code from the \texttt{pwr2ppl} (Aberson \protect\hyperlink{ref-R-pwr2ppl}{2019}) package (but Aberson's code allows for different n per condition, and different sd per condition).
...
We can also use the function in the \texttt{pwr} package (Champely \protect\hyperlink{ref-R-pwr}{2020}). Note that we need to calculate \emph{f} to use this function, which is based on the means and sd, as illustrated in the formulas above.
...
Finally, g*Power (Faul et al. \protect\hyperlink{ref-faul2007g}{2007}) provides the option to calculate \texttt{f} from the means, sd and n for the cells. It can then be used to calculate power.
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-269-1}
...
\caption{\label{tab:unnamed-chunk-270}Exact ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
We can compare these results to (Aberson \protect\hyperlink{ref-R-pwr2ppl}{2019}), as well.
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-273-1}
...
\caption{\label{tab:unnamed-chunk-274}Exact ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-277-1}
...
\caption{\label{tab:unnamed-chunk-278}Exact ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-279-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-280-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-281-1}
\pagebreak
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-282-1}
\pagebreak
...
As Potvin and Schutz (\protect\hyperlink{ref-potvin2000statistical}{2000}) write:
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-283-1}
...
\NormalTok{plot_data <-}\StringTok{ }\KeywordTok{rbind}\NormalTok{(p_a}\OperatorTok{$}\NormalTok{power_df, p_b}\OperatorTok{$}\NormalTok{power_df, p_c}\OperatorTok{$}\NormalTok{power_df, }
\NormalTok{                   p_d}\OperatorTok{$}\NormalTok{power_df, p_e}\OperatorTok{$}\NormalTok{power_df, p_f}\OperatorTok{$}\NormalTok{power_df)}\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{AxB =} \StringTok{`}\DataTypeTok{A:B}\StringTok{`}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{select}\NormalTok{(n,effect,A,B,AxB)}

\NormalTok{long_data =}\StringTok{ }\NormalTok{plot_data }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{gather}\NormalTok{(}\StringTok{"A"}\NormalTok{, }\StringTok{"B"}\NormalTok{, }\StringTok{"AxB"}\NormalTok{, }\DataTypeTok{key =}\NormalTok{ factor, }\DataTypeTok{value =}\NormalTok{ power)}

\NormalTok{plot1 <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(long_data, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ n, }\DataTypeTok{y =}\NormalTok{ power,}
                               \DataTypeTok{color =} \KeywordTok{as.factor}\NormalTok{(effect))) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_line}\NormalTok{(}\DataTypeTok{size =} \FloatTok{1.5}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{scale_y_continuous}\NormalTok{(}\DataTypeTok{breaks =} \KeywordTok{seq}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{100}\NormalTok{,}\DecValTok{10}\NormalTok{)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{color =} \StringTok{"Effect Size"}\NormalTok{,}
       \DataTypeTok{x =} \StringTok{"Sample Size per Group"}\NormalTok{,}
       \DataTypeTok{y =} \StringTok{"Power (%)"}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{scale_color_viridis_d}\NormalTok{() }\OperatorTok{+}\StringTok{ }\KeywordTok{facet_wrap}\NormalTok{( }\OperatorTok{~}\StringTok{ }\NormalTok{factor, }\DataTypeTok{ncol=}\DecValTok{2}\NormalTok{)}
...
\NormalTok{plot2 <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(long_data, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ n, }\DataTypeTok{y =}\NormalTok{ power,}
                               \DataTypeTok{color =} \KeywordTok{as.factor}\NormalTok{(effect))) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_line}\NormalTok{(}\DataTypeTok{size =} \FloatTok{1.5}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{scale_y_continuous}\NormalTok{(}\DataTypeTok{breaks =} \KeywordTok{seq}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{100}\NormalTok{,}\DecValTok{10}\NormalTok{)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{color =} \StringTok{"Effect Size"}\NormalTok{,}
...
\NormalTok{plot_data <-}\StringTok{ }\KeywordTok{rbind}\NormalTok{(p_a}\OperatorTok{$}\NormalTok{power_df, p_b}\OperatorTok{$}\NormalTok{power_df, }
\NormalTok{                   p_c}\OperatorTok{$}\NormalTok{power_df, p_d}\OperatorTok{$}\NormalTok{power_df, p_e}\OperatorTok{$}\NormalTok{power_df, p_f}\OperatorTok{$}\NormalTok{power_df)}\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{AxB =} \StringTok{`}\DataTypeTok{A:B}\StringTok{`}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{select}\NormalTok{(n,correlation,A,B,AxB)}

\NormalTok{long_data =}\StringTok{ }\NormalTok{plot_data }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{gather}\NormalTok{(}\StringTok{"A"}\NormalTok{, }\StringTok{"B"}\NormalTok{, }\StringTok{"AxB"}\NormalTok{, }\DataTypeTok{key =}\NormalTok{ factor, }\DataTypeTok{value =}\NormalTok{ power)}

\NormalTok{plot3 <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(long_data, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ n, }\DataTypeTok{y =}\NormalTok{ power,}
                               \DataTypeTok{color =} \KeywordTok{as.factor}\NormalTok{(correlation))) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_line}\NormalTok{(}\DataTypeTok{size =} \FloatTok{1.5}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{scale_y_continuous}\NormalTok{(}\DataTypeTok{breaks =} \KeywordTok{seq}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{100}\NormalTok{,}\DecValTok{10}\NormalTok{)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{color =} \StringTok{"Correlation"}\NormalTok{,}
       \DataTypeTok{x =} \StringTok{"Sample Size per Group"}\NormalTok{,}
       \DataTypeTok{y =} \StringTok{"Power (%)"}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{scale_color_viridis_d}\NormalTok{() }\OperatorTok{+}\StringTok{ }\KeywordTok{facet_wrap}\NormalTok{( }\OperatorTok{~}\StringTok{ }\NormalTok{factor, }\DataTypeTok{ncol=}\DecValTok{2}\NormalTok{)}
\CommentTok{##############}
...
\NormalTok{plot_data <-}\StringTok{ }\KeywordTok{rbind}\NormalTok{(p_a}\OperatorTok{$}\NormalTok{power_df, p_b}\OperatorTok{$}\NormalTok{power_df, p_c}\OperatorTok{$}\NormalTok{power_df, }
\NormalTok{                   p_d}\OperatorTok{$}\NormalTok{power_df, p_e}\OperatorTok{$}\NormalTok{power_df, p_f}\OperatorTok{$}\NormalTok{power_df)}\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{AxB =} \StringTok{`}\DataTypeTok{A:B}\StringTok{`}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{select}\NormalTok{(n,corr_diff,A,B,AxB)}

\NormalTok{long_data =}\StringTok{ }\NormalTok{plot_data }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{gather}\NormalTok{(}\StringTok{"A"}\NormalTok{, }\StringTok{"B"}\NormalTok{, }\StringTok{"AxB"}\NormalTok{, }\DataTypeTok{key =}\NormalTok{ factor, }\DataTypeTok{value =}\NormalTok{ power)}

\NormalTok{plot4 <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(long_data, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ n, }\DataTypeTok{y =}\NormalTok{ power,}
                               \DataTypeTok{color =} \KeywordTok{as.factor}\NormalTok{(corr_diff))) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_line}\NormalTok{(}\DataTypeTok{size =} \FloatTok{1.5}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{scale_y_continuous}\NormalTok{(}\DataTypeTok{breaks =} \KeywordTok{seq}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{100}\NormalTok{,}\DecValTok{10}\NormalTok{)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{color =} \StringTok{"Difference in Correlation"}\NormalTok{,}
       \DataTypeTok{x =} \StringTok{"Sample Size per Group"}\NormalTok{,}
       \DataTypeTok{y =} \StringTok{"Power (%)"}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{scale_color_viridis_d}\NormalTok{() }\OperatorTok{+}\StringTok{ }\KeywordTok{facet_wrap}\NormalTok{( }\OperatorTok{~}\StringTok{ }\NormalTok{factor, }\DataTypeTok{ncol=}\DecValTok{2}\NormalTok{)}
\end{Highlighting}
...
Although some recommendations have been provided to assist researchers to choose an approach to deal with violations of the homogeneity assumption (Algina and Keselman \protect\hyperlink{ref-algina_detecting_1997}{1997}), it is often unclear if these violations of the homogeneity assumption are consequential for a given study.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{design_result_violation <-}\StringTok{ }\KeywordTok{ANOVA_design}\NormalTok{(}
...
\includegraphics{SuperpowerValidation_files/figure-latex/sphercity_1-1}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# power_result_violation <- ANOVA_power(}
\CommentTok{#    design_result_violation,}
\CommentTok{#    nsims = 10000,}
\CommentTok{#    verbose = TRUE)}

\NormalTok{design_result_violation_}\DecValTok{2}\NormalTok{ <-}\StringTok{ }\KeywordTok{ANOVA_design}\NormalTok{(}
  \DataTypeTok{design =} \StringTok{"2b*2b"}\NormalTok{,}
  \DataTypeTok{n =} \KeywordTok{c}\NormalTok{(}\DecValTok{80}\NormalTok{, }\DecValTok{80}\NormalTok{, }\DecValTok{80}\NormalTok{, }\DecValTok{80}\NormalTok{),}
  \DataTypeTok{mu =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{),}
  \DataTypeTok{sd =} \KeywordTok{c}\NormalTok{(}\DecValTok{3}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{5}\NormalTok{, }\DecValTok{1}\NormalTok{),}
  \DataTypeTok{labelnames =} \KeywordTok{c}\NormalTok{(}\StringTok{"condition"}\NormalTok{, }\StringTok{"cheerful"}\NormalTok{, }\StringTok{"sad"}\NormalTok{, }\StringTok{"voice"}\NormalTok{, }\StringTok{"human"}\NormalTok{, }\StringTok{"robot"}\NormalTok{))}
\end{Highlighting}
...
\includegraphics{SuperpowerValidation_files/figure-latex/sphercity_1-2}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# power_result_violation_2 <- ANOVA_power_unequal_n(}
\CommentTok{#    design_result_violation_2,}
\CommentTok{#    nsims = 10000,}
\CommentTok{#    verbose = TRUE)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{design_result_violation_}\DecValTok{3}\NormalTok{ <-}\StringTok{ }\KeywordTok{ANOVA_design}\NormalTok{(}
  \DataTypeTok{design =} \StringTok{"4w"}\NormalTok{,}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{power_result_violation_}\DecValTok{3}\NormalTok{ <-}\StringTok{ }\KeywordTok{ANOVA_power}\NormalTok{(design_result_violation_}\DecValTok{3}\NormalTok{, }
                                        \DataTypeTok{alpha_level =} \FloatTok{0.05}\NormalTok{, }
                                        \DataTypeTok{nsims =} \DecValTok{10000}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{power_result_violation_}\DecValTok{4}\NormalTok{ <-}\StringTok{ }\KeywordTok{ANOVA_power}\NormalTok{(design_result_violation_}\DecValTok{4}\NormalTok{, }
                                        \DataTypeTok{alpha_level =} \FloatTok{0.05}\NormalTok{, }
                                        \DataTypeTok{nsims =}\NormalTok{ nsims,}
                                        \DataTypeTok{correction =} \StringTok{"GG"}\NormalTok{,}
                                        \DataTypeTok{verbose =} \OtherTok{FALSE}\NormalTok{)}
\NormalTok{power_result_violation_}\DecValTok{5}\NormalTok{ <-}\StringTok{ }\KeywordTok{ANOVA_power}\NormalTok{(design_result_violation_}\DecValTok{4}\NormalTok{, }
                                        \DataTypeTok{alpha_level =} \FloatTok{0.05}\NormalTok{, }
                                        \DataTypeTok{nsims =}\NormalTok{ nsims,}
...
In our experience, most researchers default to using a Greenhouse-Geisser adjustment for sphericity, but this may not be the most statistical efficient way of dealing with violations of sphercity. While it is tempting to simply say one approach is superior to another, that is not case when the sample size is small (Maxwell, Delaney, and Kelley (\protect\hyperlink{ref-maxwell_designing_2004}{2004}), ppg 775).
...
\caption{\label{tab:unnamed-chunk-290}Simulated MANOVA Result}
\centering
\begin{tabular}[t]{l|r}
\hline
  & power\\
...
\caption{\label{tab:unnamed-chunk-291}Simulated ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r}
\hline
  & power & effect\_size\\
...
\caption{\label{tab:unnamed-chunk-292}Simulated ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r}
\hline
  & power & effect\_size\\
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-293-1}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# power_result_hfeffect <- ANOVA_power(design_result_power, }
...
\caption{\label{tab:unnamed-chunk-295}Simulated MANOVA Result}
\centering
\begin{tabular}[t]{l|r}
\hline
  & power\\
...
\NormalTok{design_result <-}\StringTok{ }\KeywordTok{ANOVA_design}\NormalTok{(}
  \DataTypeTok{design =} \StringTok{"2w*3w"}\NormalTok{,}
  \DataTypeTok{n =} \DecValTok{40}\NormalTok{,}
  \DataTypeTok{mu =} \KeywordTok{c}\NormalTok{(}\FloatTok{0.3}\NormalTok{, }\DecValTok{0}\NormalTok{, }\FloatTok{0.5}\NormalTok{, }\FloatTok{0.3}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{),}
  \DataTypeTok{sd =} \DecValTok{2}\NormalTok{,}
  \DataTypeTok{r =} \FloatTok{0.8}\NormalTok{, }
  \DataTypeTok{labelnames =} \KeywordTok{c}\NormalTok{(}\StringTok{"condition"}\NormalTok{, }\StringTok{"cheerful"}\NormalTok{, }\StringTok{"sad"}\NormalTok{, }\StringTok{"voice"}\NormalTok{, }\StringTok{"human"}\NormalTok{, }\StringTok{"robot"}\NormalTok{, }\StringTok{"cartoon"}\NormalTok{),}
  \DataTypeTok{plot =} \OtherTok{TRUE}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-296-1}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{exact_result <-}\StringTok{ }\KeywordTok{ANOVA_exact}\NormalTok{(}
\NormalTok{  design_result,}
  \DataTypeTok{alpha_level =} \FloatTok{0.05}\NormalTok{,}
  \DataTypeTok{verbose =} \OtherTok{FALSE}\NormalTok{,}
  \DataTypeTok{emm =} \OtherTok{TRUE}\NormalTok{,}
  \DataTypeTok{contrast_type =} \StringTok{"pairwise"}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(exact_result}\OperatorTok{$}\NormalTok{main_results[}\DecValTok{1}\OperatorTok{:}\DecValTok{2}\NormalTok{])}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{((exact_result}\OperatorTok{$}\NormalTok{emm_results[}\DecValTok{1}\OperatorTok{:}\DecValTok{2}\NormalTok{]))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\KeywordTok{emmeans_power}\NormalTok{(exact_result}\OperatorTok{$}\NormalTok{emmeans}\OperatorTok{$}\NormalTok{contrasts)[}\DecValTok{1}\OperatorTok{:}\DecValTok{2}\NormalTok{])}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{simple_condition_effects <-}\StringTok{ }\KeywordTok{emmeans}\NormalTok{(}
\NormalTok{  exact_result}\OperatorTok{$}\NormalTok{emmeans}\OperatorTok{$}\NormalTok{emmeans,}
  \DataTypeTok{specs =} \OperatorTok{~}\StringTok{ }\NormalTok{condition }\OperatorTok{|}\StringTok{ }\NormalTok{voice}
\NormalTok{)}

\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\KeywordTok{emmeans_power}\NormalTok{(}\KeywordTok{pairs}\NormalTok{(simple_condition_effects))[}\DecValTok{1}\OperatorTok{:}\DecValTok{2}\NormalTok{])}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\KeywordTok{emmeans_power}\NormalTok{(}\KeywordTok{test}\NormalTok{(simple_condition_effects, }\DataTypeTok{null =} \FloatTok{0.5}\NormalTok{))[}\DecValTok{1}\OperatorTok{:}\DecValTok{2}\NormalTok{])}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{custom_contrast <-}\StringTok{ }\KeywordTok{contrast}\NormalTok{(}
\NormalTok{  exact_result}\OperatorTok{$}\NormalTok{emmeans}\OperatorTok{$}\NormalTok{emmeans,}
  \KeywordTok{list}\NormalTok{(}\DataTypeTok{robot_vs_sad_human =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\FloatTok{-0.5}\NormalTok{, }\FloatTok{-0.5}\NormalTok{))}
\NormalTok{)}

\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\KeywordTok{emmeans_power}\NormalTok{(custom_contrast))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{n_contrasts <-}\StringTok{ }\KeywordTok{nrow}\NormalTok{(}\KeywordTok{as.data.frame}\NormalTok{(simple_condition_effects))}

\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\KeywordTok{emmeans_power}\NormalTok{(}
  \KeywordTok{pairs}\NormalTok{(simple_condition_effects),}
  \DataTypeTok{alpha_level =} \FloatTok{0.05} \OperatorTok{/}\StringTok{ }\NormalTok{n_contrasts}
...
\begin{Highlighting}[]
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\KeywordTok{emmeans_power}\NormalTok{(}
  \KeywordTok{pairs}\NormalTok{(simple_condition_effects)[}\DecValTok{1}\NormalTok{],}
  \DataTypeTok{alpha_level =} \DecValTok{2} \OperatorTok{*}\StringTok{ }\FloatTok{0.05}
\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\KeywordTok{emmeans_power}\NormalTok{(}
  \KeywordTok{pairs}\NormalTok{(simple_condition_effects, }\DataTypeTok{side =} \StringTok{"equivalence"}\NormalTok{, }\DataTypeTok{delta =} \FloatTok{0.3}\NormalTok{)[}\DecValTok{2}\NormalTok{]}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{voice_by_condition <-}\StringTok{ }\KeywordTok{joint_tests}\NormalTok{(}
\NormalTok{  exact_result}\OperatorTok{$}\NormalTok{emmeans}\OperatorTok{$}\NormalTok{emmeans,}
  \DataTypeTok{by =} \StringTok{"condition"}
\NormalTok{)}

\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\KeywordTok{emmeans_power}\NormalTok{(voice_by_condition, }\DataTypeTok{alpha_level =} \FloatTok{0.05} \OperatorTok{/}\StringTok{ }\DecValTok{2}\NormalTok{)[}\DecValTok{1}\OperatorTok{:}\DecValTok{3}\NormalTok{])}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{monte_result1 <-}\StringTok{ }\KeywordTok{ANOVA_power}\NormalTok{(}
\NormalTok{  design_result,}
  \DataTypeTok{alpha_level =} \FloatTok{0.05}\NormalTok{,}
  \DataTypeTok{verbose =} \OtherTok{FALSE}\NormalTok{,}
  \DataTypeTok{emm =} \OtherTok{TRUE}\NormalTok{,}
  ...
  \DataTypeTok{nsims =} \DecValTok{10000}\NormalTok{, }\CommentTok{#set total number of simulations}
  \DataTypeTok{seed =} \DecValTok{27042020}\NormalTok{, }\CommentTok{#set seed for reproducibility}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(monte_result1}\OperatorTok{$}\NormalTok{emm_results[}\DecValTok{1}\OperatorTok{:}\DecValTok{2}\NormalTok{])}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{x <-}\StringTok{ }\KeywordTok{rep}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\DecValTok{10}\NormalTok{)}
\CommentTok{# Three random grouping variables }
\CommentTok{# (e.g., Petri dishes)}
\NormalTok{g <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{'a'}\NormalTok{, }\StringTok{'b'}\NormalTok{, }\StringTok{'c'}\NormalTok{)}

\CommentTok{# Comine into 1 dataset}
\NormalTok{X <-}\StringTok{ }\KeywordTok{expand.grid}\NormalTok{(}\DataTypeTok{x=}\NormalTok{x, }\DataTypeTok{g=}\NormalTok{g)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{b <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{, }\FloatTok{-0.1}\NormalTok{) }
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{model1 <-}\StringTok{ }\KeywordTok{makeLmer}\NormalTok{(y }\OperatorTok{~}\StringTok{ }\NormalTok{x }\OperatorTok{+}\StringTok{ }\NormalTok{(}\DecValTok{1}\OperatorTok{|}\NormalTok{g),}
                   \DataTypeTok{fixef=}\NormalTok{b, }
                   \DataTypeTok{VarCorr=}\NormalTok{V1, }
...
## Formula: y ~ x + (1 | g)
##    Data: X
## REML criterion at convergence: 82.8826
## Random effects:
##  Groups   Name        Std.Dev.
##  g        (Intercept) 0.7071  
##  Residual             1.0000  
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sim1 =}\StringTok{ }\KeywordTok{powerSim}\NormalTok{(model1, }
                \DataTypeTok{nsim=}\DecValTok{20}\NormalTok{, }
...
\CommentTok{## props = proportions in each group (length is equal to number of groups)}
\NormalTok{gen_bidat =}\StringTok{ }\ControlFlowTok{function}\NormalTok{(props,N)\{}
  \CommentTok{# Create data with N participants and balanced assignment to treatment group}
\NormalTok{  df =}\StringTok{ }\KeywordTok{genData}\NormalTok{(N) }\OperatorTok{%>%}\StringTok{ }\CommentTok{# generate data with N participants}
\StringTok{    }\KeywordTok{trtAssign}\NormalTok{(}\DataTypeTok{n =} \KeywordTok{length}\NormalTok{(props),}
              \DataTypeTok{balanced =} \OtherTok{TRUE}\NormalTok{,}
              \DataTypeTok{grpName =} \StringTok{"trt"}\NormalTok{) }\CommentTok{# Randomly assign treatment (tr) in a balanced fashion  }
  \CommentTok{# Get the number of gropus by the number or proportions entered}
\NormalTok{  grps =}\StringTok{ }\KeywordTok{length}\NormalTok{(props)}
  ...
  \ControlFlowTok{for}\NormalTok{(i }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\NormalTok{grps)\{ }\CommentTok{# run loop once for each group in the study}
\NormalTok{    grp =}\StringTok{ }\NormalTok{i}\DecValTok{-1} \CommentTok{# the simulation runs from 1 to the total number of groups}
    \CommentTok{# the i-1 starts the loop at zero}
    \CommentTok{# We then assign the group (grp) by which number in the loop we are in}
\NormalTok{    con_run =}\StringTok{ }\KeywordTok{paste0}\NormalTok{(}\StringTok{"trt == "}\NormalTok{, grp)}
...
\NormalTok{      dc =}\StringTok{ }\KeywordTok{defCondition}\NormalTok{(}
        \DataTypeTok{condition =}\NormalTok{ con_run,}
        \DataTypeTok{formula =}\NormalTok{ props[i],}
        \DataTypeTok{dist =} \StringTok{"binary"}\NormalTok{,}
        \DataTypeTok{link =} \StringTok{"identity"}
\NormalTok{      )}
...
\NormalTok{  dat <-}\StringTok{ }\KeywordTok{addCondition}\NormalTok{(}\DataTypeTok{condDefs =}\NormalTok{ dc,}
                      \DataTypeTok{dtOld =}\NormalTok{ df,}
                      \DataTypeTok{newvar =} \StringTok{"y"}\NormalTok{)}
  \KeywordTok{return}\NormalTok{(dat)}
\NormalTok{\}}

\CommentTok{# Test run}
\KeywordTok{gen_bidat}\NormalTok{(}\KeywordTok{c}\NormalTok{(.}\DecValTok{2}\NormalTok{,.}\DecValTok{35}\NormalTok{),}\DataTypeTok{N=}\DecValTok{10}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pwr_bidat =}\StringTok{ }\ControlFlowTok{function}\NormalTok{(props,N,}
                     \DataTypeTok{reps=}\DecValTok{100}\NormalTok{,}
...
\NormalTok{  sims =}\StringTok{ }\KeywordTok{replicate}\NormalTok{(}\DataTypeTok{n =}\NormalTok{ reps,}
                   \KeywordTok{gen_bidat}\NormalTok{(props, }\DataTypeTok{N =}\NormalTok{ N),}
                   \DataTypeTok{simplify =} \OtherTok{FALSE}\NormalTok{)}
  \CommentTok{# Run an equivalent number of analyses}
\NormalTok{  sims2 =}\StringTok{ }\NormalTok{purrr}\OperatorTok{::}\KeywordTok{map}\NormalTok{(sims,  }\OperatorTok{~}\StringTok{ }\KeywordTok{glm}\NormalTok{(y }\OperatorTok{~}\StringTok{ }\NormalTok{trt,}
                                  \DataTypeTok{data =}\NormalTok{ .x,}
                                  \DataTypeTok{family =} \KeywordTok{binomial}\NormalTok{(}\DataTypeTok{link =} \StringTok{"logit"}\NormalTok{)))}
  \CommentTok{# Get the summary coefficients from our models}
\NormalTok{  sims3 =}\StringTok{ }\NormalTok{purrr}\OperatorTok{::}\KeywordTok{map}\NormalTok{(sims2,  }\OperatorTok{~}\StringTok{ }\KeywordTok{summary}\NormalTok{(.x)}\OperatorTok{$}\NormalTok{coefficients)}
  \CommentTok{# Put all the results into a data frame (tibble)}
\NormalTok{  sims4 =}\StringTok{ }\NormalTok{purrr}\OperatorTok{::}\KeywordTok{map}\NormalTok{(sims3,}
                     \OperatorTok{~}\StringTok{ }\NormalTok{tibble}\OperatorTok{::}\KeywordTok{rownames_to_column}\NormalTok{(}\KeywordTok{as.data.frame}\NormalTok{(.x),}
                                                  \StringTok{"coef"}\NormalTok{))}
  \CommentTok{# Combine all the data frames into one}
\NormalTok{  simsdf =}\StringTok{ }\KeywordTok{bind_rows}\NormalTok{(sims4, }\DataTypeTok{.id =} \StringTok{"nrep"}\NormalTok{)}
  \CommentTok{# Summarize results by coefficient}
\NormalTok{  simspow =}\StringTok{ }\NormalTok{simsdf }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{group_by}\NormalTok{(coef) }\OperatorTok{%>%}
\StringTok{    }\CommentTok{# Calculate the power (number of results with p-value < alpha)}
\StringTok{    }\KeywordTok{summarize}\NormalTok{(}
      \DataTypeTok{estimate =} \KeywordTok{mean}\NormalTok{(Estimate),}
      \DataTypeTok{power =} \KeywordTok{mean}\NormalTok{(}\StringTok{`}\DataTypeTok{Pr(>|z|)}\StringTok{`} \OperatorTok{<}\StringTok{ }\NormalTok{alpha),}
      \CommentTok{# Calculate confidence intervals}
      \DataTypeTok{power.lci =} \KeywordTok{prop.test}\NormalTok{(}\KeywordTok{sum}\NormalTok{(}\StringTok{`}\DataTypeTok{Pr(>|z|)}\StringTok{`} \OperatorTok{<}\StringTok{ }\NormalTok{alpha), reps)}\OperatorTok{$}\NormalTok{conf.int[}\DecValTok{1}\NormalTok{],}
      \DataTypeTok{power.uci =} \KeywordTok{prop.test}\NormalTok{(}\KeywordTok{sum}\NormalTok{(}\StringTok{`}\DataTypeTok{Pr(>|z|)}\StringTok{`} \OperatorTok{<}\StringTok{ }\NormalTok{alpha), reps)}\OperatorTok{$}\NormalTok{conf.int[}\DecValTok{2}\NormalTok{],}
      \DataTypeTok{.groups =} \StringTok{'drop'}
\NormalTok{    )}
  
  \CommentTok{# Return table of results}
  \KeywordTok{return}\NormalTok{(simspow)}
\NormalTok{\}}

\KeywordTok{set.seed}\NormalTok{(}\DecValTok{01292020}\NormalTok{)}
\NormalTok{pwr_res =}\StringTok{ }\KeywordTok{pwr_bidat}\NormalTok{(}\KeywordTok{c}\NormalTok{(.}\DecValTok{4}\NormalTok{,.}\DecValTok{3}\NormalTok{),}\DataTypeTok{N=}\DecValTok{1000}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(pwr_res }\OperatorTok{%>%}
\StringTok{               }\KeywordTok{rename}\NormalTok{(}\DataTypeTok{Coefficients =}\NormalTok{ coef,}
                      \StringTok{`}\DataTypeTok{Average Log Odds}\StringTok{`}\NormalTok{ =}\StringTok{ }\NormalTok{estimate,}
                      \DataTypeTok{Power =}\NormalTok{ power,}
                      \StringTok{`}\DataTypeTok{Lower C.I.}\StringTok{`}\NormalTok{ =}\StringTok{ }\NormalTok{power.lci,}
                      \StringTok{`}\DataTypeTok{Upper C.I.}\StringTok{`}\NormalTok{ =}\StringTok{ }\NormalTok{power.uci),}
             \DataTypeTok{digits =} \DecValTok{2}\NormalTok{,}
             \DataTypeTok{caption =} \StringTok{"Result from Power Simulation"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{table}

\caption{\label{tab:unnamed-chunk-316}Result from Power Simulation}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pwr_}\DecValTok{2}\NormalTok{ =}\StringTok{ }\KeywordTok{pwr_bidat}\NormalTok{(}\KeywordTok{c}\NormalTok{(.}\DecValTok{035}\NormalTok{,.}\DecValTok{0175}\NormalTok{),}\DataTypeTok{N=}\DecValTok{2000}\NormalTok{)}

\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(pwr_}\DecValTok{2}\NormalTok{,}
             \DataTypeTok{caption =} \StringTok{"Another Study of Binary Outcomes"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{table}

\caption{\label{tab:unnamed-chunk-317}Another Study of Binary Outcomes}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
...
\NormalTok{nLooks <-}\StringTok{ }\DecValTok{3} 
\CommentTok{# Here is where you list the information fraction (% of total N)}
\CommentTok{## (e.g. here 50%, 75% and 100% information)}
\NormalTok{analyses_scheduled <-}\StringTok{ }\NormalTok{(}\KeywordTok{c}\NormalTok{(}\FloatTok{0.50}\NormalTok{, }\FloatTok{0.75}\NormalTok{, }\DecValTok{1}\NormalTok{))}
\NormalTok{efficacy_thresholds <-}\StringTok{ }\KeywordTok{numeric}\NormalTok{(nLooks)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{design <-}\StringTok{ }\KeywordTok{getDesignGroupSequential}\NormalTok{(}\DataTypeTok{sided =} \DecValTok{2}\NormalTok{,}
                                   \DataTypeTok{alpha =} \FloatTok{0.05}\NormalTok{,}
                                   \DataTypeTok{informationRates =}\NormalTok{ analyses_scheduled,}
                                   \DataTypeTok{typeOfDesign =} \StringTok{"asOF"}\NormalTok{)}
\ControlFlowTok{for}\NormalTok{(j }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\NormalTok{nLooks)\{}
\NormalTok{  efficacy_thresholds[j] =}\StringTok{ }\NormalTok{design}\OperatorTok{$}\NormalTok{stageLevels[j]}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pwr_bidat2 =}\StringTok{ }\ControlFlowTok{function}\NormalTok{(props, N,}
                      \DataTypeTok{interim_n =} \KeywordTok{c}\NormalTok{(}\DecValTok{500}\NormalTok{,}\DecValTok{750}\NormalTok{,}\DecValTok{1000}\NormalTok{),}
                      \DataTypeTok{reps =} \DecValTok{100}\NormalTok{,}
                      \DataTypeTok{alpha =}\NormalTok{ efficacy_thresholds,}
                      \DataTypeTok{conf.level =} \FloatTok{.95}\NormalTok{)\{}
  \CommentTok{#Create empty data frame for the results of each repetition }
\NormalTok{  res =}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{N =} \KeywordTok{rep}\NormalTok{(}\OtherTok{NA}\NormalTok{,reps),}
                   \DataTypeTok{alpha =} \KeywordTok{rep}\NormalTok{(}\OtherTok{NA}\NormalTok{,reps),}
                   \DataTypeTok{success =} \KeywordTok{rep}\NormalTok{(}\OtherTok{NA}\NormalTok{,reps),}
                   \DataTypeTok{stop =} \KeywordTok{rep}\NormalTok{(}\OtherTok{NA}\NormalTok{,reps),}
                   \DataTypeTok{p.value =} \KeywordTok{rep}\NormalTok{(}\OtherTok{NA}\NormalTok{,reps),}
                   \DataTypeTok{estimate =} \KeywordTok{rep}\NormalTok{(}\OtherTok{NA}\NormalTok{,reps))}
...
\NormalTok{    dat =}\StringTok{ }\KeywordTok{gen_bidat}\NormalTok{(props, }\DataTypeTok{N =}\NormalTok{ N) }
    \CommentTok{# Set logical indicator for stopping the trial}
\NormalTok{    stop_trial =}\StringTok{ }\OtherTok{FALSE}
    \CommentTok{# Run loop for each potential stopping point}
    \ControlFlowTok{for}\NormalTok{(j }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\KeywordTok{length}\NormalTok{(interim_n))\{}
      \CommentTok{# Set current N and alpha level}
\NormalTok{      val =}\StringTok{ }\NormalTok{interim_n[j]}
\NormalTok{      alpha_j =}\StringTok{ }\NormalTok{alpha[j]}
      \CommentTok{# Only continue analysis if stopping criteria not met}
      \ControlFlowTok{if}\NormalTok{(stop_trial }\OperatorTok{!=}\StringTok{ }\OtherTok{TRUE}\NormalTok{)\{}
        \CommentTok{# Get interim data points}
\NormalTok{        dat_i =}\StringTok{ }\NormalTok{dat[}\DecValTok{1}\OperatorTok{:}\NormalTok{val,]}
        \CommentTok{# Build model}
\NormalTok{        mod_i =}\StringTok{ }\KeywordTok{glm}\NormalTok{(y }\OperatorTok{~}\StringTok{ }\NormalTok{trt,}
                    \DataTypeTok{data =}\NormalTok{ dat_i,}
                    \DataTypeTok{family =} \KeywordTok{binomial}\NormalTok{(}\DataTypeTok{link =} \StringTok{"logit"}\NormalTok{))}
        \CommentTok{# Get results from the model}
\NormalTok{        sum_i =}\StringTok{ }\KeywordTok{summary}\NormalTok{(mod_i)}\OperatorTok{$}\NormalTok{coefficients[}\DecValTok{2}\NormalTok{,]}
        
        \CommentTok{# Determine if the trial can stop}
        \ControlFlowTok{if}\NormalTok{(sum_i[}\DecValTok{4}\NormalTok{] }\OperatorTok{<}\StringTok{ }\NormalTok{alpha_j)\{}
\NormalTok{          stop_trial =}\StringTok{ }\OtherTok{TRUE}
\NormalTok{          alpha_f =}\StringTok{ }\NormalTok{alpha_j}
\NormalTok{          n_f =}\StringTok{ }\NormalTok{val}
\NormalTok{        \}}
\NormalTok{      \}}
\NormalTok{    \}}
    
    \CommentTok{# Get final alpha and N if no "success" at final N}
    \ControlFlowTok{if}\NormalTok{(stop_trial }\OperatorTok{==}\StringTok{ }\OtherTok{FALSE}\NormalTok{)\{}
\NormalTok{      n_f =}\StringTok{ }\NormalTok{val}
\NormalTok{      alpha_f =}\StringTok{ }\NormalTok{alpha_j}
\NormalTok{    \}}
    \CommentTok{# Store results from this repetition}
\NormalTok{    res}\OperatorTok{$}\NormalTok{N[i] =}\StringTok{ }\NormalTok{n_f}
\NormalTok{    res}\OperatorTok{$}\NormalTok{alpha[i] =}\StringTok{ }\NormalTok{alpha_f}
\NormalTok{    res}\OperatorTok{$}\NormalTok{success[i] =}\StringTok{ }\NormalTok{stop_trial}
\NormalTok{    res}\OperatorTok{$}\NormalTok{stop[i] =}\StringTok{ }\KeywordTok{ifelse}\NormalTok{(stop_trial,n_f,}\StringTok{"fail"}\NormalTok{)}
\NormalTok{    res}\OperatorTok{$}\NormalTok{p.value[i] =}\StringTok{ }\NormalTok{sum_i[}\DecValTok{4}\NormalTok{]}
\NormalTok{    res}\OperatorTok{$}\NormalTok{estimate[i] =}\StringTok{ }\NormalTok{sum_i[}\DecValTok{1}\NormalTok{]}
\NormalTok{  \}}
  \CommentTok{# Create summary of results}
\NormalTok{  simspow =}\StringTok{ }\NormalTok{res }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{summarize}\NormalTok{(}
      \DataTypeTok{med_est =} \KeywordTok{median}\NormalTok{(estimate),}
      \DataTypeTok{power =} \KeywordTok{mean}\NormalTok{(p.value }\OperatorTok{<}\StringTok{ }\NormalTok{alpha),}
      \CommentTok{# Calculate confidence intervals}
      \DataTypeTok{power.lci =} \KeywordTok{prop.test}\NormalTok{(}\KeywordTok{sum}\NormalTok{(p.value }\OperatorTok{<}\StringTok{ }\NormalTok{alpha), reps)}\OperatorTok{$}\NormalTok{conf.int[}\DecValTok{1}\NormalTok{],}
      \DataTypeTok{power.uci =} \KeywordTok{prop.test}\NormalTok{(}\KeywordTok{sum}\NormalTok{(p.value }\OperatorTok{<}\StringTok{ }\NormalTok{alpha), reps)}\OperatorTok{$}\NormalTok{conf.int[}\DecValTok{2}\NormalTok{]}
\NormalTok{    )}
  
\NormalTok{  stoppow =}\StringTok{ }\NormalTok{res }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{group_by}\NormalTok{(stop) }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{summarize}\NormalTok{(}
      \DataTypeTok{prop =} \KeywordTok{n}\NormalTok{()}\OperatorTok{/}\NormalTok{reps,}
      \DataTypeTok{prop.lci =} \KeywordTok{prop.test}\NormalTok{(}\KeywordTok{n}\NormalTok{(), reps)}\OperatorTok{$}\NormalTok{conf.int[}\DecValTok{1}\NormalTok{],}
      \DataTypeTok{prop.uci =} \KeywordTok{prop.test}\NormalTok{(}\KeywordTok{n}\NormalTok{(), reps)}\OperatorTok{$}\NormalTok{conf.int[}\DecValTok{2}\NormalTok{],}
      \DataTypeTok{mean.est =} \KeywordTok{mean}\NormalTok{(estimate),}
      \DataTypeTok{.groups =} \StringTok{'drop'}
\NormalTok{    )}
  
  
  \KeywordTok{return}\NormalTok{(}\KeywordTok{list}\NormalTok{(}\DataTypeTok{sims =}\NormalTok{ res,}
              \DataTypeTok{power_tab =}\NormalTok{ simspow,}
              \DataTypeTok{stop_tab =}\NormalTok{ stoppow))}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{20210324}\NormalTok{)}
\NormalTok{test =}\StringTok{ }\KeywordTok{pwr_bidat2}\NormalTok{(}\DataTypeTok{props =} \KeywordTok{c}\NormalTok{(.}\DecValTok{3}\NormalTok{,.}\DecValTok{4}\NormalTok{),}
                  \DataTypeTok{N =} \DecValTok{1000}\NormalTok{)}
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(test}\OperatorTok{$}\NormalTok{power_tab,}
             \DataTypeTok{caption =} \StringTok{"Power for Interim Analysis Design"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{table}

\caption{\label{tab:unnamed-chunk-321}Power for Interim Analysis Design}
\centering
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{test}\OperatorTok{$}\NormalTok{stop_tab }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{stop =} \KeywordTok{factor}\NormalTok{(stop,}
                       \DataTypeTok{levels =} \KeywordTok{c}\NormalTok{(}\StringTok{"fail"}\NormalTok{, }\StringTok{"500"}\NormalTok{,}\StringTok{"750"}\NormalTok{,}\StringTok{"1000"}\NormalTok{))) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ stop, }\DataTypeTok{color =}\NormalTok{ stop, }\DataTypeTok{y =}\NormalTok{ prop)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_point}\NormalTok{(}\DataTypeTok{size=}\DecValTok{2}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_errorbar}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{ymin =}\NormalTok{ prop.lci,}
                    \DataTypeTok{ymax =}\NormalTok{ prop.uci)) }\OperatorTok{+}\StringTok{ }
\StringTok{  }\KeywordTok{coord_flip}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-322-1}
...
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{stop =} \KeywordTok{factor}\NormalTok{(stop,}
                       \DataTypeTok{levels =} \KeywordTok{c}\NormalTok{(}\StringTok{"fail"}\NormalTok{, }\StringTok{"500"}\NormalTok{,}\StringTok{"750"}\NormalTok{,}\StringTok{"1000"}\NormalTok{))) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ stop, }\DataTypeTok{color =}\NormalTok{ stop, }\DataTypeTok{y =}\NormalTok{ mean.est)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_point}\NormalTok{(}\DataTypeTok{size =} \DecValTok{2}\NormalTok{) }
\end{Highlighting}
\end{Shaded}

\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-323-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/aberson_appendix-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/aberson_5.3.2-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/aberson_5.3-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/aberson_6.2-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/aberson_6.6-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/aberson_6.8-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/aberson_7.2-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-341-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-342-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-343-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-344-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-345-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-346-1}
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-352-1}
...
\caption{\label{tab:unnamed-chunk-354}Simulated ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r}
\hline
  & power & effect\_size\\
...
\caption{\label{tab:unnamed-chunk-356}Exact ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-363-1}
...
\caption{\label{tab:unnamed-chunk-365}Simulated ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r}
\hline
  & power & effect\_size\\
...
\caption{\label{tab:unnamed-chunk-367}Exact ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-374-1}
...
\caption{\label{tab:unnamed-chunk-376}Simulated ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r}
\hline
  & power & effect\_size\\
...
\caption{\label{tab:unnamed-chunk-378}Exact ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-380-1}
...
\caption{\label{tab:unnamed-chunk-382}Simulated ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r}
\hline
  & power & effect\_size\\
...
\caption{\label{tab:unnamed-chunk-384}Exact ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
\includegraphics{SuperpowerValidation_files/figure-latex/unnamed-chunk-391-1}
...
\caption{\label{tab:unnamed-chunk-393}Simulated ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r}
\hline
  & power & effect\_size\\
...
\caption{\label{tab:unnamed-chunk-395}Exact ANOVA Result}
\centering
\begin{tabular}[t]{l|r|r|r|r}
\hline
  & power & partial\_eta\_squared & cohen\_f & non\_centrality\\
...
\hypertarget{refs}{}
\leavevmode\hypertarget{ref-R-pwr2ppl}{}%
Aberson, Chris. 2019. \emph{Pwr2ppl: Power Analyses for Common Designs (Power to the People)}. \url{https://CRAN.R-project.org/package=pwr2ppl}.

\leavevmode\hypertarget{ref-albers2018power}{}%
Albers, Casper, and Daniël Lakens. 2018. ``When Power Analyses Based on Pilot Data Are Biased: Inaccurate Effect Size Estimators and Follow-up Bias.'' \emph{Journal of Experimental Social Psychology} 74: 187--95. \url{https://doi.org/10.1016/j.jesp.2017.09.004}.

\leavevmode\hypertarget{ref-algina_detecting_1997}{}%
Algina, James, and H. J. Keselman. 1997. ``Detecting Repeated Measures Effects with Univariate and Multivariate Statistics.'' \emph{Psychological Methods} 2 (2): 208--18. \url{https://doi.org/10.1037/1082-989X.2.2.208}.
...
\leavevmode\hypertarget{ref-Althouse2021}{}%
Althouse, Andrew D. 2021. ``Post Hoc Power: Not Empowering, Just Misleading.'' \emph{Journal of Surgical Research} 259 (March): A3--A6. \url{https://doi.org/10.1016/j.jss.2019.10.049}.

\leavevmode\hypertarget{ref-brysbaert2019many}{}%
Brysbaert, Marc. 2019. ``How Many Participants Do We Have to Include in Properly Powered Experiments? A Tutorial of Power Analysis with Reference Tables.'' \emph{Journal of Cognition} 2 (1). \url{http://doi.org/10.5334/joc.72}.

\leavevmode\hypertarget{ref-Caldwell2020}{}%
Caldwell, Aaron, and Andrew D. Vigotsky. 2020. ``A Case Against Default Effect Sizes in Sport and Exercise Science.'' \emph{PeerJ} 8 (November): e10314. \url{https://doi.org/10.7717/peerj.10314}.

\leavevmode\hypertarget{ref-Campbell2012MorePower6F}{}%
Campbell, Jamie, and Valerie A. Thompson. 2012. ``MorePower 6.0 for Anova with Relational Confidence Intervals and Bayesian Analysis.'' \emph{Behavior Research Methods} 44: 1255--65. \url{https://doi.org/10.3758/s13428-012-0186-0}.

\leavevmode\hypertarget{ref-R-pwr}{}%
Champely, Stephane. 2020. \emph{Pwr: Basic Functions for Power Analysis}. \url{https://github.com/heliosdrm/pwr}.

\leavevmode\hypertarget{ref-cohen1988spa}{}%
...
Faul, Franz, Edgar Erdfelder, Albert-Georg Lang, and Axel Buchner. 2007. ``G*Power 3: A Flexible Statistical Power Analysis Program for the Social, Behavioral, and Biomedical Sciences.'' \emph{Behavior Research Methods} 39 (2): 175--91. \url{http://www.psychologie.hhu.de/arbeitsgruppen/allgemeine-psychologie-und-arbeitspsychologie/gpower.html}.

\leavevmode\hypertarget{ref-gelman2019don}{}%
Gelman, Andrew. 2019. ``Don't Calculate Post-Hoc Power Using Observed Estimate of Effect Size.'' \emph{Annals of Surgery} 269 (1): e9--e10.

\leavevmode\hypertarget{ref-Green2016}{}%
Green, Peter, and Catriona J. MacLeod. 2016. ``SIMR : An R Package for Power Analysis of Generalized Linear Mixed Models by Simulation.'' Edited by Shinichi Nakagawa. \emph{Methods in Ecology and Evolution} 7 (4): 493--98. \url{https://doi.org/10.1111/2041-210x.12504}.

\leavevmode\hypertarget{ref-R-multcomp}{}%
Hothorn, Torsten, Frank Bretz, and Peter Westfall. 2020. \emph{Multcomp: Simultaneous Inference in General Parametric Models}. \url{https://CRAN.R-project.org/package=multcomp}.

\leavevmode\hypertarget{ref-Lakens2021}{}%
Lakens, Daniel, Friedrich Pahlke, and Gernot Wassmer. 2021. ``Group Sequential Designs: A Tutorial,'' January. \url{https://doi.org/10.31234/osf.io/x4azm}.

\leavevmode\hypertarget{ref-Lakens2014}{}%
Lakens, Daniël. 2014. ``Performing High-Powered Studies Efficiently with Sequential Analyses.'' \emph{European Journal of Social Psychology} 44 (7): 701--10. \url{https://doi.org/10.1002/ejsp.2023}.

\leavevmode\hypertarget{ref-Lenth2007}{}%
Lenth, Russell V. 2007. ``Statistical Power Calculations.'' \emph{Journal of Animal Science} 85 (suppl\_13): E24--E29.

\leavevmode\hypertarget{ref-maxwell_designing_2004}{}%
Maxwell, S. E., H. D. Delaney, and K. Kelley. 2004. \emph{Designing Experiments and Analyzing Data: A Model Comparison Perspective}. Avec CD, v. 1. Lawrence Erlbaum Associates. \url{https://designingexperiments.com/}.

\leavevmode\hypertarget{ref-muller1984practical}{}%
Muller, Keith E, and Bercedis L Peterson. 1984. ``Practical Methods for Computing Power in Testing the Multivariate General Linear Hypothesis.'' \emph{Computational Statistics \& Data Analysis} 2 (2): 143--58. \url{https://doi.org/10.1016/0167-9473(84)90002-1}.

\leavevmode\hypertarget{ref-o1999pragmatic}{}%
O'Brien, Ralph G, and Gwowen Shieh. 1999. ``Pragmatic, Unifying Algorithm Gives Power Probabilities for Common F Tests of the Multivariate General Linear Hypothesis.'' Citeseer. \url{https://pdfs.semanticscholar.org/deda/facfd686feea985e134babc524ceb46f0422.pdf}.

\leavevmode\hypertarget{ref-pavot1993affective}{}%
Pavot, William, and Ed Diener. 1993. ``The Affective and Cognitive Context of Self-Reported Measures of Subjective Well-Being.'' \emph{Social Indicators Research} 28 (1): 1--20.

\leavevmode\hypertarget{ref-Perugini2014}{}%
Perugini, Marco, Marcello Gallucci, and Giulio Costantini. 2014. ``Safeguard Power as a Protection Against Imprecise Power Estimates.'' \emph{Perspectives on Psychological Science} 9 (3): 319--32.

\leavevmode\hypertarget{ref-potvin2000statistical}{}%
Potvin, Patrick J, and Robert W Schutz. 2000. ``Statistical Power for the Two-Factor Repeated Measures Anova.'' \emph{Behavior Research Methods, Instruments, \& Computers} 32 (2): 347--56. \url{https://psycnet.apa.org/doi/10.3758/BF03207805}.

\leavevmode\hypertarget{ref-R-base}{}%
R Core Team. 2020. \emph{R: A Language and Environment for Statistical Computing}. Vienna, Austria: R Foundation for Statistical Computing. \url{https://www.R-project.org/}.

\leavevmode\hypertarget{ref-SASglmpower}{}%
SAS. 2015. ``The Glmpower Procedure.'' In \emph{SAS/Stat 14.1 User's Guide}, 3738--98. Cary, NC: SAS Institute Inc. \url{http://support.sas.com/documentation/cdl/en/statug/68162/HTML/default/viewer.htm\#statug_glmpower_toc.htm}.

\leavevmode\hypertarget{ref-simonsohn_2014}{}%
Simonsohn, Uri. 2014. ``No-Way Interactions.'' \emph{Data Colada}. \url{http://datacolada.org/17}.

\leavevmode\hypertarget{ref-vonesh1986sample}{}%
Vonesh, Edward F, and M Anthony Schork. 1986. ``Sample Sizes in the Multivariate Analysis of Repeated Measurements.'' \emph{Biometrics}, 601--10. \url{http://dx.doi.org/10.2307/2531210}.
